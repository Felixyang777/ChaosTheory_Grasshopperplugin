...
namespace ChaosTheory
{
    public class DadrasAttractor : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (!DA.GetData(2, ref Sigma1)) return;
            if (!DA.GetData(3, ref Gama)) return;
            if (!DA.GetData(4, ref Sigma2)) return;
            ...
            List<Point3d> LorenzOscillatorPoints = GenerateDadrasAttractor(StartPoint, Rou, Sigma1, Gama, Sigma2, Epsilon, DeltaT, Iterations);
        ...
        Point3d point;
        List<Point3d> GenerateDadrasAttractor(Point3d StartPoint, double Rou, double Sigma1, double Gama, double Sigma2, double Epsilon, double DeltaT, int Iterations)
        {
            point = StartPoint;
            ...
            for (int i = 0; i < Iterations; i++)
            {
                ...
                newpoints.Add(point);
                double dx = y-Rou*x+Sigma1*y*z;
                double dy = Gama*y-x*z+z;
                double dz = Sigma2*x*y-Epsilon*z;
