...
{
    public class YuWangAttractor : GH_Component
    {
        public YuWangAttractor()
          : base("YuWangAttractor", "AA",
            "Construct a YuWangAttractor.",
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("Beta", "β", "Beta", GH_ParamAccess.item, 40);
            pManager.AddNumberParameter("Sigma", "ς", "Sigma", GH_ParamAccess.item, 2);
            pManager.AddNumberParameter("Delta", "δ", "Delta", GH_ParamAccess.item, 2.5);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            List<Point3d> YuWangAttractorPoints = GenerateYuWangAttractor(StartPoint, Alpha, Beta, Sigma, Delta, DeltaT, Iterations);
            IEnumerable __enum_points = (IEnumerable)YuWangAttractorPoints;
            DA.SetDataList(0, __enum_points);

            var curve = Curve.CreateInterpolatedCurve(YuWangAttractorPoints, 3);
            DA.SetData(1, curve);
        ...
        Point3d point;
        List<Point3d> GenerateYuWangAttractor(Point3d StartPoint, double Alpha, double Beta, double Sigma, double Delta, double DeltaT, int Iterations)
        {
            point = StartPoint;
            ...
            for (int i = 0; i < Iterations; i++)
            {
                ...
                newpoints.Add(point);
                double dx = Alpha * (y - x);
                double dy = Beta * x -Sigma* x * z;
                double dz = Math.Pow(Math.E,x*y)-Delta*z;
        ...
        public override Guid ComponentGuid => new Guid("6A7E9B4C-1F3D-4A8D-9C6E-5B3A7D1F4E8C");
    }
}