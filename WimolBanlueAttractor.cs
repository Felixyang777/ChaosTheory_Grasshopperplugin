...
{
    public class WimolBanlueAttractor : GH_Component
    {
        public WimolBanlueAttractor()
          : base("WimolBanlueAttractor", "AA",
            "Construct a WimolBanlueAttractor.",
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            List<Point3d> WimolBanlueAttractorPoints = GenerateWimolBanlueAttractor(StartPoint, Alpha, DeltaT, Iterations);
            IEnumerable __enum_points = (IEnumerable)WimolBanlueAttractorPoints;
            DA.SetDataList(0, __enum_points);

            var curve = Curve.CreateInterpolatedCurve(WimolBanlueAttractorPoints, 3);
            DA.SetData(1, curve);
        ...
        Point3d point;
        List<Point3d> GenerateWimolBanlueAttractor(Point3d StartPoint, double Alpha, double DeltaT, int Iterations)
        {
            point = StartPoint;
            ...
            for (int i = 0; i < Iterations; i++)
            {
                ...
                newpoints.Add(point);
                double dx = y-x;
                double dy = -z*Math.Tanh(x);
                double dz = -Alpha+x*y+Math.Abs(y);
        ...
        public override Guid ComponentGuid => new Guid("8F3A9E1C-4D7B-4A2C-9D6E-5B1A8C4F7E3D");
    }
}