...
{
    public class AnishchenkoAstakhovAttractor : GH_Component
    {
        public AnishchenkoAstakhovAttractor()
          : base("AnishchenkoAstakhovAttractor", "AA",
            "Construct a AnishchenkoAstakhovAttractor.",
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (!DA.GetData(1, ref Miu)) return;
            ...
            List<Point3d> AnishchenkoAstakhovAttractorPoints = GenerateAnishchenkoAstakhovAttractor(StartPoint, Miu, Eta, DeltaT, Iterations);
            IEnumerable __enum_points = (IEnumerable)AnishchenkoAstakhovAttractorPoints;
            DA.SetDataList(0, __enum_points);

            var curve = Curve.CreateInterpolatedCurve(AnishchenkoAstakhovAttractorPoints, 3);
            DA.SetData(1, curve);
        ...
        Point3d point;
        List<Point3d> GenerateAnishchenkoAstakhovAttractor(Point3d StartPoint, double Miu, double Eta, double DeltaT, int Iterations)
        {
            point = StartPoint;
            ...
            for (int i = 0; i < Iterations; i++)
            {
                ...
                int Ix = (x < 0) ? 1 : 0;
                double dx = Miu+x+y-x*z;
                double dy = -x;  
                double dz = -Eta *z+Eta*Ix*Math.Pow(x,2);
        ...
        public override Guid ComponentGuid => new Guid("F27A5E3C-9B84-4D12-A6C5-8E3F1B7D4A29");
    }
}